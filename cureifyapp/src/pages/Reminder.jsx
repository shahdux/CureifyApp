...
// ─── Local medicines database ─────────────────────────────────────────────────
const MEDICINES_DB = [
    // Antibiotics
    "Vibramycin", "Amoxicillin", "Augmentin", "Zithromax", "Flagyl",
    "Ciprofloxacin", "Doxycycline", "Clindamycin", "Azithromycin", "Metronidazole",
    // Pain & Fever
    "Panadol", "Ibuprofen", "Brufen", "Voltaren", "Aspirin", "Cataflam",
    "Diclofenac", "Arcoxia", "Celebrex", "Tramadol",
    // Stomach & Digestion
    "Omeprazole", "Nexium", "Pantoprazole", "Gaviscon", "Motilium",
    "Buscopan", "Smecta", "Lactulose", "Duphalac", "Sebionex Trio cream",
    // Heart & Blood Pressure
    "Concor", "Norvasc", "Coveram", "Aprovel", "Coversyl",
    "Atorvastatin", "Crestor", "Lipitor", "Warfarin", "Plavix",
    // Diabetes
    "Metformin", "Glucophage", "Lantus", "Januvia", "Sebofix",
    "Jardiance", "Forxiga", "Victoza", "Diamicron", "Glucotrol",
    // Vitamins & Supplements
    "Omega-3", "Vitamin C", "Vitamin D", "Vitamin B12", "Zinc",
    "Calcium", "Magnesium", "Folic Acid", "Iron", "Multivitamin",
    // Skin
    "Sebionex", "Differin", "Dalacin", "Retin-A", "Clindasol",
    // Allergy
    "Claritin", "Zyrtec", "Allegra", "Telfast", "Aerius",
    "Loratadine", "Cetirizine", "Fexofenadine", "Benadryl", "Clarityne",
    // Thyroid
    "Eltroxin", "Synthroid", "Levothyroxine", "Carbimazole", "Neomercazole",
    // Other common
    "Brufen", "Prednisolone", "Dexamethasone", "Hydrocortisone", "Betamethasone",
];

// ─── Fuzzy name corrector ─────────────────────────────────────────────────────
const correctMedName = (rawName) => {
    // Strip dosage like "100mg", "500ml" to get just the name
    const nameOnly = rawName.replace(/\d+\s*(mg|ml|mcg|g|iu|%)/gi, '').trim();

    let bestMatch = null;
    let bestScore = 0;

    MEDICINES_DB.forEach(med => {
        const a = nameOnly.toLowerCase();
        const b = med.toLowerCase();

        // Character-by-character similarity score
        let matches = 0;
    ...
    // Only replace if similarity is above 60%
    if (bestScore > 0.6 && bestMatch) {
        const dosagePart = rawName.match(/\d+\s*(mg|ml|mcg|g|iu|%)/gi)?.[0] || '';
        return dosagePart ? `${bestMatch} ${dosagePart}` : bestMatch;
    }

    // No close match found — keep the AI's original output
    return rawName;
};
...
If you cannot read the prescription clearly, return an empty array: []`,
            ...
            const text = data.choices?.[0]?.message?.content || '[]';
            // Strip any accidental markdown formatting the AI may have added
            const clean = text.replace(/```json|```/g, '').trim();
            const meds = JSON.parse(clean);

            // ── Cross-reference each medicine name against local DB ──
            const correctedMeds = meds.map(med => ({
                ...med,
                medName: correctMedName(med.medName),
            }));

            setAnalyzing(false);
            navigate('/extracted', { state: { meds: correctedMeds } });
        } catch (err) {
            console.error('Groq API error:', err);
            setAnalyzing(false);
            alert(isArabic ? 'فشل تحليل الوصفة. حاول مجدداً.' : 'Failed to analyze prescription. Please try again.');
